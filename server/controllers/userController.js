...
const registerUser = asyncHandler(async (req, res) => {
  ...
  if (!name || !email || !password || !role || !title) {
    return res.status(400).json({ status: false, message: "All fields are required: name, email, password, role, title" });
  }
...
// PUT - change user password (BUG FIXED: removed hardcoded user ID block)
