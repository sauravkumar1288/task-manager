...
const Card = ({ label, count, bg, icon, sub }) => {
  return (
    ...
    <div className='h-full py-4'>
      <>
        ...
        <div className='grid grid-cols-1 md:grid-cols-4 gap-5'>
          {stats?.map(({ icon, bg, label, total, sub }, index) => (
            <Card key={index} icon={icon} bg={bg} label={label} count={total} sub={sub} />
          ))}
        </div>

        {/* Overdue Tasks Section */}
        {data?.overdueCount > 0 && (
          <div className='w-full bg-red-50 border border-red-200 my-6 p-4 rounded shadow-sm'>
            <h4 className='text-lg text-red-600 font-bold mb-3 flex items-center gap-2'>
              <MdOutlineWarning />
            ...
            <div className='flex flex-col gap-2'>
              {data?.overdueTasks?.map((task) => (
                <div
                  key={task._id}
                  className='flex items-center justify-between bg-white border border-red-100 rounded p-3'
                >
                  ...
                  <div className='flex items-center gap-3'>
                    <span className='text-xs capitalize text-gray-500 bg-gray-100 px-2 py-1 rounded'>
                      {task.priority}
                    </span>
                    <span className='text-xs text-red-500'>
                      Due: {moment(task.date).format("MMM D, YYYY")}
                    </span>
                  </div>
                </div>
              ))}
              {data?.overdueCount > 5 && (
                <p className='text-sm text-red-400 mt-1'>
                  + {data.overdueCount - 5} more overdue tasks
                </p>
              )}
            </div>
          </div>
...
const TaskTable = ({ tasks }) => {
  ...
  return (
    <div className={clsx("w-full bg-white px-2 md:px-4 pt-4 pb-4 shadow-md rounded", user?.isAdmin ? "md:w-2/3" : "")}>
